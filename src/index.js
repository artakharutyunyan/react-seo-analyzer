...
const RULES = {
  ...
  titleLength: () => {
    ...
    if (len < 30)
      return {
        ...
        severity: 'warning',
        category: 'Meta',
        message: `<title> is too short (${len} chars). Aim for 30–60.`,
      };
    if (len > 60)
      return {
        id: 'titleLength',
        severity: 'warning',
        category: 'Meta',
        message: `<title> is too long (${len} chars). Keep under 60.`,
      };
  },
  ...
  missingDescription: () => {
    ...
    if (!m || !m.getAttribute('content')?.trim())
      return {
  ...
  descriptionLength: () => {
    ...
    if (len > 160)
      return {
        ...
        severity: 'warning',
        category: 'Meta',
        message: `Meta description too long (${len} chars). Keep under 160.`,
      };
  },
  ...
  multipleH1: () => {
    ...
    if (h1s.length > 1)
      return {
        ...
        severity: 'warning',
        category: 'Structure',
        message: `Found ${h1s.length} <h1> tags. Use only one per page.`,
      };
  },
  ...
  imagesWithoutAlt: () => {
    ...
    if (imgs.length > 0)
      return {
        ...
        message: `${imgs.length} image(s) missing alt attribute.`,
      };
  },
  ...
  robotsNoIndex: () => {
    ...
    if (r && /noindex/i.test(r.getAttribute('content') || ''))
      return {
        id: 'robotsNoIndex',
        severity: 'error',
        category: 'Indexability',
        message: 'Page has robots noindex — will not be indexed.',
      };
  },
    ...
    for (const h of headings) {
      ...
      if (level - lastLevel > 1 && lastLevel !== 0)
        return {
          ...
          severity: 'warning',
          category: 'Structure',
          message: `Heading hierarchy skips levels (e.g. h${lastLevel} → h${level}).`,
        };
      lastLevel = level;
    }
  },
  ...
  return (
    <div style={panelStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ color: '#94a3b8', fontSize: 11 }}>
              ...
              {infos > 0 && (
                <span style={{ color: '#3b82f6' }}>ℹ {infos} info</span>
              )}
              {issues.length === 0 && (
      ...
      <div style={{ padding: '8px 0' }}>
        {visible.length === 0 && (
          <div
            style={{ padding: '16px', color: '#22c55e', textAlign: 'center' }}
          >
...
function Badge({ score, errorCount, onClick }) {
  return (
    ...
    >
      <span style={{ fontWeight: 700, color: scoreColor(score), fontSize: 14 }}>
        {score}
      </span>
...
export default function SEOAnalyzer({
  ...
  const runAnalysis = useCallback(() => {
    ...
    if (logToConsole) {
      if (results.length === 0) {
        console.log(
          '%c SEO Analyzer ✓ No issues detected ',
          'background:#22c55e;color:#fff;padding:2px 6px;border-radius:4px;font-weight:bold;',
        );
      } else {
        console.group(
          '%c SEO Analyzer — Score: ' + s + '/100 ',
          'background:#0f172a;color:#e2e8f0;padding:2px 6px;border-radius:4px;font-weight:bold;',
        );
        results.forEach((i) => {
          const styles = {
            error: 'color:#ef4444',
          ...
          console.warn(
            `%c[${i.severity.toUpperCase()}] ${i.message}`,
            styles[i.severity],
          );
        });
        console.groupEnd();
